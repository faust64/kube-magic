...
module "aws-vpc" {
  ...
  aws_avail_zones          = slice(data.aws_availability_zones.available.names, 0, (length(var.aws_cidr_subnets_public) > 1 ? 2 : 1))
...
resource "aws_instance" "bastion-server" {
  ...
  availability_zone           = element(slice(data.aws_availability_zones.available.names, 0, (length(var.aws_cidr_subnets_public) > 1 ? 2 : 1)), count.index)
